#dlib-lable-supervisor training
...
def visualize_landmark(image_array, landmarks, counter):
    ...
    origin_img.save("./processed/% d.jpg" % counter)
    # imshow(origin_img)



# def video_to_image(videoFile):
...
if __name__=='__main__':
    ...
    while ret:
        counter+=1
        face,landmarks = face_preprocess(image = frame,
                                         landmark_model_type='large',
                                         crop_size=280)

        visualize_landmark(image_array=face,landmarks=landmarks, counter = counter)
        np.save("./landmark_data/%d" % counter, landmarks)
        ret, frame = cap.read()
